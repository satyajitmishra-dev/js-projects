...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let roleIndex = 0;
    let charIndex = 0;
    const devRoleElement = document.getElementById("dev-role");

    function type() {
        if (charIndex < roles[roleIndex].length) {
            devRoleElement.textContent += roles[roleIndex].charAt(charIndex);
            charIndex++;
            setTimeout(type, 100);
        } else {
    ...
    function erase() {
        if (charIndex > 0) {
            devRoleElement.textContent = roles[roleIndex].substring(0, charIndex - 1);
            charIndex--;
            setTimeout(erase, 50);
        } else {
